<template>
  <div class="admin-page">
    <div class="page-header">
      <h1 class="page-title">取消报名管理</h1>
    </div>

    <div class="stats-row">
      <div class="stat-card">
        <div class="stat-value">{{ totalCount }}</div>
        <div class="stat-label">总取消数</div>
      </div>
      <div class="stat-card">
          ...
          <table>
            ...
            <tbody>
              ...
              <tr v-for="item in paginatedData" :key="item.id">
                ...
                <td>{{ formatDateTime(item.cancel_time) }}</td>
                <td>
                  <span :class="['status-tag', getStatusClass(item.audit_status)]">
                    {{ getStatusText(item.audit_status) }}
                  </span>
                </td>
                <td>
                  <button v-if="item.audit_status === 0" class="btn btn-success btn-sm" @click="audit(item, 1)">
                    <i class="fas fa-check"></i> 通过
                  </button>
                  <button v-if="item.audit_status === 0" class="btn btn-danger btn-sm" @click="audit(item, 2)">
    ...
    <div class="modal" v-if="showDetailModal" @click="closeDetailModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3 class="modal-title">取消申请详情</h3>
        ...
        <div class="modal-body" v-if="selectedItem">
          ...
          <div class="detail-row">
            ...
            <div class="detail-value">{{ selectedItem.activity_title }}</div>
          </div>
          <div class="detail-row">
            <div class="detail-label">取消原因</div>
            <div class="detail-value">{{ selectedItem.cancel_reason || '-' }}</div>
          </div>
          <div class="detail-row">
            <div class="detail-label">取消时间</div>
            <div class="detail-value">{{ formatDateTime(selectedItem.cancel_time) }}</div>
          </div>
          <div class="detail-row">
            <div class="detail-label">审核状态</div>
            <div class="detail-value">
              <span :class="['status-tag', getStatusClass(selectedItem.audit_status)]">
                {{ getStatusText(selectedItem.audit_status) }}
              </span>
            </div>
          </div>
          <div class="detail-row" v-if="selectedItem.audit_remark">
            <div class="detail-label">审核意见</div>
            <div class="detail-value">{{ selectedItem.audit_remark }}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    filteredData() {
      ...
      if (this.filterStatus !== '') {
        data = data.filter(item => item.audit_status === parseInt(this.filterStatus))
      }
      return data
    },
    paginatedData() {
    ...
    pendingCount() {
      return this.cancellations.filter(item => item.audit_status === 0).length
    },
    approvedCount() {
      return this.cancellations.filter(item => item.audit_status === 1).length
    },
    rejectedCount() {
      return this.cancellations.filter(item => item.audit_status === 2).length
    }
  },
  mounted() {
    this.loadData()
  ...
  methods: {
    async loadData() {
      this.loading = true
      try {
        const response = await api.get('/api/admin/medical/cancellations')
    ...
    async audit(item, status) {
      ...
      if (!confirm(`确定要${confirmText}该取消申请吗？`)) return

      this.loading = true
      try {
        await api.post(`/api/admin/medical/cancellations/${item.id}/audit`, {
          audit_status: status,
          audit_remark: status === 2 ? '审核不通过' : ''
        })
